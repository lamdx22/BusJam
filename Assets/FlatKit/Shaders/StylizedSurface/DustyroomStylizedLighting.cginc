...
void surfObject(InputObject IN, inout SurfaceOutputDustyroom o) {
    ...
    // duongrs
    // added to calculate viewDir for orthorgraphic camera
    float3 viewDir = lerp(IN.viewDir, normalize(UNITY_MATRIX_V._m20_m21_m22), unity_OrthoParams.w);

    half4 c = SurfaceCore(blendedNormal, IN.worldPos, IN.lightDir, viewDir);
