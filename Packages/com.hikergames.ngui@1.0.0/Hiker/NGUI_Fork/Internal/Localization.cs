...
static public class Localization
{
	...
	public static bool multilingual = false;
	...
	static public void ShouldReset()
    {
        localizationHasBeenSet = false;
		mLanguage = null;
		mLanguageIndex = -1;
	...
	static bool LoadDictionary (string value, bool merge = false)
	{
        ...
        if (!localizationHasBeenSet)
		{
			...
			{
				asset = Resources.Load<TextAsset>("Localization");
		...
		/// duongrs changed 21.02.2025
		/// remove support single language old style
		/// so we only load in csv mode
		bool re = LoadCSV(bytes, merge);
		if (asset != null)
        {
	...
	static bool LoadCSV (byte[] bytes, TextAsset asset, bool merge = false)
	{
		...
		if (!localizationHasBeenSet || (!merge && !mMerging) || mLanguages == null || mLanguages.Length == 0)
		{
            ...
            mLanguages = new string[multilingual ? header.size : (mLanguage == header.buffer[0] ? 1 : 2)];
            ...
            {
                mLanguages[0] = header.buffer[0];
				curLangIndex = 0;
				mLanguageIndex = 0;
                for (int i = 1; i < header.size; ++i)
                {
                    if (header.buffer[i] == mLanguage)
                    {
                        curLangIndex = i;
						mLanguages[1] = header.buffer[i];
						mLanguageIndex = 1;
						break;
                    }
                }
...
#if DEBUG
            Debug.Log("Init languages: " + string.Join(",", mLanguages));
#endif
        }
        else
        {
	...
	static string[] ExtractStrings (BetterList<string> added, string[] newLanguages, Dictionary<string, int> languageIndices)
	{
		...
		else
		{
			...
			for (int i = 0, imax = newLanguages.Length; i < imax; ++i)
			{
				...
				if (languageIndices.ContainsKey(language))
				{
