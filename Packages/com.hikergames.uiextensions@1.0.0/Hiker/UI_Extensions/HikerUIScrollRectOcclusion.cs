...
namespace UnityEngine.UI.Extensions
{
    ...
    [AddComponentMenu("Hiker/UI/Extensions/Hiker UI Scrollrect Occlusion")]
    public class HikerUIScrollRectOcclusion : MonoBehaviour
    {
        ...
        public Vector2 customSizeItem = Vector2.zero;

        float _disableMarginYMin = -0.5f;
        float _disableMarginYMax = 0.5f;
        float _disableMarginXMin = -0.5f;
        float _disableMarginXMax = 0.5f;

        public bool IsInitialized() { return _initialised; }
        ...
        void ToggleGridComponents(bool toggle)
        {
            if (_isVertical)
            {
                var offset = Mathf.Approximately(customSizeItem.y, 0) ? _items[0].sizeDelta.y : customSizeItem.y;

                var rectTran = _scrollRect.GetComponent<RectTransform>();
                var rect = rectTran.rect;

                _disableMarginYMin = rect.yMin - offset;
                _disableMarginYMax = rect.yMax + offset;
            }
                

            if (_isHorizontal)
            {
                var offset = Mathf.Approximately(customSizeItem.x, 0) ? _items[0].sizeDelta.x : customSizeItem.x;

                var rectTran = _scrollRect.GetComponent<RectTransform>();
                var rect = rectTran.rect;

                _disableMarginXMin = rect.xMin - offset;
                _disableMarginXMax = rect.xMax + offset;
            }
        ...
        public void OnScroll(Vector2 pos)
        {
            ...
            {
                var itemPos = _scrollRect.transform.InverseTransformPoint(_items[i].position);
                if (_isVertical && _isHorizontal)
                {
                    if (itemPos.y < _disableMarginYMin || itemPos.y > _disableMarginYMax ||
                        itemPos.x < _disableMarginXMin || itemPos.x > _disableMarginXMax)
                    {
                ...
                else
                {
                    ...
                    if (_isHorizontal)
                    {
                        if (itemPos.x < _disableMarginXMin || itemPos.x > _disableMarginXMax)
                        {
                            _items[i].gameObject.SetActive(false);
                        }
                        else
