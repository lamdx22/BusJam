...
namespace Hiker.UI
{
    ...
    public class HikerScrollRect : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
    {
        ...
        [SerializeField]
        private RectTransform[] m_BoundExclude;
        ...
        public void CenterOnRectTransform(RectTransform rectTrans, bool clamp)
        {
            CenterOnRectTransform(rectTrans, 0, clamp);
            CenterOnRectTransform(rectTrans, 1, clamp);
        }

        public void CenterOnRectTransform(RectTransform rectTran, int axis, bool clamp)
        {
            var normalizeVal = GetNormalizePosition(rectTran, axis);
            SetNormalizedPosition(clamp ? Mathf.Clamp01(normalizeVal) : normalizeVal, axis);
        }
        ...
        public bool hNeedScrollContent
        {
            get { return m_ContentBounds.size.x > m_ViewBounds.size.x + 0.01f; }
        }

        public bool vNeedScrollContent
        {
            get { return m_ContentBounds.size.y > m_ViewBounds.size.y + 0.01f; }
        }
        ...
        public Bounds GetBounds()
        {
            ...
            if (m_CheckAllBoundContent && m_Content)
            {
                for (int i = 0; i < m_Content.childCount; ++i)
                {
                    ...
                    if (m_BoundExclude == null || System.Array.Exists(m_BoundExclude, e => e == child) == false)
                    {
        ...
        public bool SetExcludeTransform(RectTransform trans)
        {
            bool success = false;
            if (m_BoundExclude == null || m_BoundExclude.Length == 0)
            {
                m_BoundExclude = new RectTransform[1] { trans };
                success = true;
            }

            if (success == false)
            {
                for (int i = 0; i < m_BoundExclude.Length; ++i)
                {
                    if (m_BoundExclude[i] == trans)
                    {
                        success = true;
                        break;
                    }
            ...
            if (success == false)
            {
                for (int i = 0; i < m_BoundExclude.Length; ++i)
                {
                    if (m_BoundExclude[i] == null)
                    {
                        m_BoundExclude[i] = trans;
                        success = true;
                        break;
                    }
                }
            ...
            if (success == false)
            {
                int oldSize = m_BoundExclude.Length;
                System.Array.Resize<RectTransform>(ref m_BoundExclude, oldSize + 1);
                m_BoundExclude[oldSize] = trans;
                SetDirty();
            }
            return success;
        }
